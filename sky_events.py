...
def build_report_text(all_events, iss_events, args) -> str:
    lines = []
    lines.append("Sky Events Report")
    lines.append("=" * 60)
    lines.append(f"Generated: {now_local().strftime('%a %b %d, %I:%M %p %Z')}")
    lines.append(f"Location: lat={args.lat}, lon={args.lon} | alt {args.alt}m")
    lines.append(f"Horizon:   {args.days} days")
    lines.append("")
    ...
    def fmt(e):
        ...
        if e.get("end"):
            ...
            return f"- {e.get('type','Event')}: {e['name']} - {when} -> {end}"
        return f"- {e.get('type','Event')}: {e['name']} - {when}"
    
    lines.append("Astronomy events:")
    if all_events:
        for e in all_events:
            lines.append(fmt(e))
    else:
